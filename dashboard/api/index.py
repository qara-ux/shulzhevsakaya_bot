...
async def get_stats(db: Session = Depends(get_db)):
    ...
    # Funnel steps
    starts = tot
    engagement = db.query(func.count(func.distinct(AnalyticsEvent.user_id))).filter(AnalyticsEvent.event_name == "click_start").scalar() or 0
    leads = db.query(func.count(func.distinct(AnalyticsEvent.user_id))).filter(AnalyticsEvent.event_name == "payment_started").scalar() or 0
